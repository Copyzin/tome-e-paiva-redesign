...
- **Astro 6, `output: 'static'`, clean directory URLs.** Pre-rendered HTML; the only client JS is two inline scripts (Navbar nav behavior + Hero effects, see below).
...
- **Mobile is a faithful port of the design's phone mockup** (`design-reference/project/ui_kits/website/mobile-preview.css`), implemented as real `@media (max-width: 640px)` rules in `site/src/styles/mobile.css` (loaded after `site.css`). On phones the hero keeps its **parallax symbol** as faded background art (`.tp-hero__logo` scaled up to `96vw`, `opacity .55`), stacks the corner-anchored copy with comfortable gaps, and renders the two CTAs **full-width** — the cursor parallax/spotlight just rest on touch (no fallback that changes layout). `mobile.css` also enforces `overflow-x: hidden` (the hero symbol bleeds off-screen by design).
- **Components** in `site/src/components/*.astro` are 1:1 ports of `design-reference/project/ui_kits/website/*.jsx`. Most are static. **Interactivity is vanilla `<script>`, not React:**
  - `Navbar.astro` — scroll-glass (`.is-scrolled` at `scrollY > 80`), mobile sheet menu, body scroll-lock. The Especialidades dropdown is pure CSS `:hover`/`:focus-within`. The brand is a gold-gradient **lockup** (`.tp-logo`: symbol + "TOMÉ & PAIVA" with a display-font `&` + letter-spread "advogados associados"); on ≤1024px only the symbol shows (`.tp-logo__words` hidden).
  - `Hero.astro` — gradient backdrop with a cursor-tracked **parallax brand symbol**, a **spotlight** that lights only the logo artwork (radial mask × the PNG's gold alpha), and a **golden-dust** `<canvas>`. The text sits in corner-anchored blocks (`.tp-hero__layout` top/bottom). Effects honor `prefers-reduced-motion` and pause offscreen via `IntersectionObserver`; on touch (no cursor) they simply rest. No React, no 3D card.
